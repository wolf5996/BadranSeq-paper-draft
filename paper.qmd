...
This design preserves compatibility with existing Seurat workflows whilst eliminating the repetitive figure polishing steps that researchers currently perform manually.

The package embodies a "sensible defaults" philosophy: every function produces publication-ready output without requiring additional styling or parameter tuning.
Unlike many specialised visualisation packages that require learning new syntax or data structures, BadranSeq leverages the familiar Seurat object interface and ggplot2 grammar.
Users invoke a single function call where they might previously have written 10-15 lines of customisation code, yet every returned plot remains a standard ggplot object for optional downstream modification.

This approach addresses a specific gap in the Seurat ecosystem: while Seurat excels at data processing and exploratory analysis, its plotting functions prioritise flexibility over presentation quality.
The result is that researchers spend substantial time on repetitive styling tasks---adding variance labels to PCA axes, applying consistent themes, configuring colour scales for readability---rather than focusing on biological interpretation.
BadranSeq automates these decisions using established best practices from the visualisation literature, whilst preserving the underlying flexibility of the ggplot2 system.

The architecture follows a routing pattern where `do_DimPlot()` serves as a unified entry point, automatically dispatching to specialised functions (`do_PcaPlot()`, `do_UmapPlot()`, etc.) based on the requested reduction type.
This design allows users to switch between visualisation types without learning different function signatures, whilst enabling each specialised function to implement reduction-specific enhancements such as automatic variance annotation for PCA or optimised colour scaling for feature plots.
...
# Advanced: custom silhouette transparency and border control
p3 <- do_UmapPlot(seurat_obj, split.by = "condition", 
                  silhouette_alpha = 0.3, plot_cell_borders = TRUE)
```
