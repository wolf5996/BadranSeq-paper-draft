...
`EnhancedElbowPlot()` provides variance-explained diagnostics with an optional cutoff overlay, enabling direct comparison with Seurat's default elbow output (@fig-elbow).
...
The breadth of functionality in visualisation packages introduces a corresponding dependency burden that affects installation time, version compatibility, and long-term maintenance. BadranSeq maintains a minimal dependency tree with only 19 direct dependencies compared to 56 for SCpubr---a 66\% reduction (@fig-dependency). This streamlined approach reduces the risk of version conflicts, accelerates installation, and lowers the maintenance overhead for users while delivering equivalent publication-ready aesthetics for core visualisation tasks.
