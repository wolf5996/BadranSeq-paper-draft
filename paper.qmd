...
![Comprehensive UMAP comparison showing Seurat (top row) versus BadranSeq (bottom row). Seurat's DimPlot() produces basic exploratory plots lacking cell borders, consistent themes, and publication styling. BadranSeq's do_UmapPlot() delivers publication-ready output with automatic cell borders, boxed cluster labels, vivid categorical colours, and integrated theming without additional configuration.](badranseq_joss_figures/write/figures/fig1_comparison.svg){#fig-comparison width="100%"}
...
![Statistical violin comparison showing Seurat's VlnPlot() (left) versus BadranSeq's do_StatsViolinPlot() (right). Seurat displays expression distributions without statistical annotation. BadranSeq integrates Kruskal--Wallis omnibus testing and Dunn's pairwise comparisons with automatic formatting, enabling immediate statistical interpretation without manual post-processing.](badranseq_joss_figures/write/figures/fig6_stats_violin.svg){#fig-violin width="100%"}
