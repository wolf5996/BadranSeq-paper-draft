...
This figure compares distribution visualisation between Seurat's `VlnPlot()`
and BadranSeq's `do_StatsViolinPlot()`, highlighting the integrated statistical
testing capabilities.

- **Left**: Seurat's VlnPlot — basic violin without statistical annotation
- **Right**: BadranSeq's do_StatsViolinPlot — automatic Kruskal–Wallis test
  and Dunn's pairwise comparisons
...
This figure quantifies the dependency burden difference between BadranSeq and
SCpubr, illustrating the trade-off between functionality and maintenance overhead.

- **BadranSeq**: 19 direct dependencies (minimal footprint)
- **SCpubr**: 56 direct dependencies (broad functionality)
- **Implication**: Reduced installation time, fewer version conflicts, lower
